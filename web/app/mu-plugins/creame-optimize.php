...
add_action('admin_head', function(){ remove_action('admin_notices', 'update_nag', 3); }, 1);

// Remove WordPress logo on admin bar
add_action('add_admin_bar_menus', function(){ remove_action('admin_bar_menu', 'wp_admin_bar_wp_menu'); });

// Disable Heartbeat API
// add_action('init', function(){ wp_deregister_script('heartbeat'); }, 1);
// Or change Heartbeat interval setting
add_filter('heartbeat_settings', function($settings){ return array_merge($settings, ['interval' => 60]); });
...
if (defined('WP_DEFAULT_THEME')) {
    add_filter('wp_prepare_themes_for_js', function($themes){ return array_intersect_key($themes, [WP_DEFAULT_THEME => 1]); });
}
...
add_filter('post_mime_types', function($mime_types){
    $mime_types['application/pdf'] = [__('PDF'), __('Manage PDF'), _n_noop('PDF <span class="count">(%s)</span>', 'PDF <span class="count">(%s)</span>')];
    return $mime_types;
});
...
function creame_clean_header(){
    ...
    add_filter('the_generator', '__return_empty_string');                 // Removes the generator name from the RSS feeds
...
function creame_remove_version($src){
    return is_admin() ? $src : remove_query_arg('ver', $src);
}
add_filter('style_loader_src', 'creame_remove_version');
add_filter('script_loader_src', 'creame_remove_version');
...
add_filter('embed_oembed_html', function($output){ return str_replace('https://www.youtube.com/', 'https://www.youtube-nocookie.com/', $output); }, 10);
...
if (WP_ENV === 'production') add_filter('robots_txt', function($robots){
    file_put_contents(dirname(ABSPATH).'/robots.txt', $robots, LOCK_EX);
    return $robots;
}, 100);

// Remove robots.txt
add_action('creame_optimize_jobs', function(){ wp_delete_file(dirname(ABSPATH).'/robots.txt'); });
